...
public class GameMap {
		...
		while (scanner.hasNextLine() && (tempStr = scanner.nextLine()) != null) {
			...
			if (!tempStr.startsWith("#")) {
				...
				if (tempStrArr.length == 2) {
					...
					GlobalEnvironment.GAMEMAPE_WIDTH = width;
					GlobalEnvironment.GAMEMAPE_HEIGHT = height;
	...
	public static GameMap getGameMap2(GameMap gm) {
		...
		for (Obstacle o : gm.getObstacles()) {
			...
			o2 = new Obstacle(o.getType(), o.getName(), new Point(left, top), new Point(right, top), new Point(left, bottom), new Point(right, bottom));
		...
		for (int i = 0; i < horizontalNum; i++) {
			for (int k = 0; k < verticalNum; k++) {
				...
				for (Obstacle o : GlobalEnvironment.gameMap2.getObstacles()) {
					...
					if (!((ox1 <= x1 && ox2 <= x1) || (ox1 >= x2 && ox2 >= x2) || (oy1 <= y1 && oy2 <= y1) || (oy1 >= y2 && oy2 >= y2))) {
						obstacleList.add(o);
					}
				}
